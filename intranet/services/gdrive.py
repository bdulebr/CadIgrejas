...
* ARQUIVO: intranet/services/gdrive.py
...
def upload_backup_to_gdrive(file_path='db.sqlite3'):
    ...
    if not settings.GDRIVE_FOLDER_ID:
        raise ValueError("ERRO: 'GDRIVE_FOLDER_ID' não está configurado no arquivo .env. Como as Contas de Serviço do Google Workspace não possuem cota de armazenamento próprio, você DEVE fornecer o ID de uma pasta que esteja em um 'Drive Compartilhado' (Shared Drive) para receber o arquivo.")
